...
public class MatroskaFileWriter implements Closeable
{
  ...
  private MasterElement chaptersElem;
  ...
  /**
   * Sets the Chapters element to be written at close time. The element must be
   * a fully constructed {@code Chapters} master element containing
   * {@code EditionEntry} / {@code ChapterAtom} children.
   *
   * @param chapters a {@link MatroskaDocTypes#Chapters} master element
   */
  public void setChapters(final MasterElement chapters)
  {
    this.chaptersElem = chapters;
  }
  ...
  public void close()
  {
    ...
    if (ioDW.isSeekable())
    {
      ...
      {
        metaSeek.addIndexedElement(MatroskaDocTypes.Chapters.getType(), ioDW.getFilePointer());
        chaptersElem.writeElement(ioDW);
      }
